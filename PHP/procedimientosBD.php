...
//UPDATE `salioviajeuy_salioviajeuy`.`faqs` SET `Respuesta` = 'respuesta' WHERE (`idPregunta` = '38');
    ...
    public function traer_viajes_cotizando_por_id($id)
    {
        ...
        if ($stmt->execute()) {
            ...
            while ($stmt->fetch()) {
                $result = array('ID' => $id_cotizacion, 'DIRECCION_ORIGEN' => $dir_origen, 'BARRIO_ORIGEN' => $bar_origen, 'LOCALIDAD_ORIGEN' => $loc_origen, 'DIRECCION_DESTINO' => $dir_destino, 'BARRIO_DESTINO' => $bar_destino, 'LOCALIDAD_DESTINO' => $loc_destino, 'FECHA_SALIDA' => $fechaSalida, 'ESTADO' => $estado, 'HORA' => $hora, 'CANTIDAD_PASAJEROS' => $cantidad_pasajeros, 'MASCOTAS' => $mascotas,'TIPO' => $tipo,'OBSERVACIONES' => $obs,'MASCOTAS' => $mascotas,'DURACION' => $len,'NRO_BARCO_VUELO' => $nro_b_v,'EQUIPAJE' => $nro_equi);
                $fecha = $result["FECHA_SALIDA"];
                $timestamp = strtotime($fecha); 
                $newDate = date("d-m-Y", $timestamp);  
                $result["FECHA_SALIDA"] = $newDate;
