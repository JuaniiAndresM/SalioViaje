...
//UPDATE `salioviajeuy_salioviajeuy`.`faqs` SET `Respuesta` = 'respuesta' WHERE (`idPregunta` = '38');
    ...
    public function traer_viajes_cotizando_panel_admin()
    {
        $choferes = array();
        $conn = $this->conexion();
        $query = "SELECT ID,DIRECCION_ORIGEN,BARRIO_ORIGEN,LOCALIDAD_ORIGEN,DIRECCION_DESTINO,BARRIO_DESTINO,LOCALIDAD_DESTINO,FECHA_SALIDA,ESTADO,HORA,CANTIDAD_PASAJEROS,MASCOTAS,TIPO FROM `cotizaciones` ORDER BY FECHA_SALIDA";
        $stmt = $conn->prepare($query);
        if ($stmt->execute()) {
            $stmt->store_result();
    ...
    public function traer_viajes_cotizando_por_id($id)
    {
        ...
        if ($stmt->execute()) {
            ...
            while ($stmt->fetch()) {
                $result = array('ID' => $id_cotizacion, 'DIRECCION_ORIGEN' => $dir_origen, 'BARRIO_ORIGEN' => $bar_origen, 'LOCALIDAD_ORIGEN' => $loc_origen, 'DIRECCION_DESTINO' => $dir_destino, 'BARRIO_DESTINO' => $bar_destino, 'LOCALIDAD_DESTINO' => $loc_destino, 'FECHA_SALIDA' => $fechaSalida, 'ESTADO' => $estado, 'HORA' => $hora, 'CANTIDAD_PASAJEROS' => $cantidad_pasajeros, 'MASCOTAS' => $mascotas,'TIPO' => $tipo,'OBSERVACIONES' => $obs,'MASCOTAS' => $mascotas,'DURACION' => $len,'NRO_BARCO_VUELO' => $nro_b_v,'EQUIPAJE' => $nro_equi,'HORA_REGRESO' => $hr_regreso,'FECHA_REGRESO' => $fechaRegreso,'ID_SOLICITANTE' => $id_soli);
                $fecha = $result["FECHA_SALIDA"];
                $timestamp = strtotime($fecha); 
                $newDate = date("d-m-Y", $timestamp);  
                $result["FECHA_SALIDA"] = $newDate;
