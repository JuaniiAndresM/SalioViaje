...
//UPDATE `salioviajeuy_salioviajeuy`.`faqs` SET `Respuesta` = 'respuesta' WHERE (`idPregunta` = '38');
    ...
    public function traer_oportunidades_por_id_usuario($id)
    {
        ...
        $oportunidades = array();
        $conn = $this->conexion();
        $query = "SELECT idViaje,Origen,Destino,Fecha,Estado,Modalidad FROM viajes where idComprador = $id and Modalidad = 'Oportunidad' and visivilidad != 0 ORDER BY Fecha, Hora;";
        $stmt = $conn->prepare($query);
        if ($stmt->execute()) {
            $stmt->store_result();
    ...
    public function traer_oportunidades_por_id_tta($id)
    {
        ...
        $oportunidades = array();
        $conn = $this->conexion();
        $query = "SELECT idViaje,Origen,Destino,Fecha,Estado,Modalidad FROM viajes where idTransportista = $id and visivilidad != 0 ORDER BY Fecha, Hora;";
        $stmt = $conn->prepare($query);
        if ($stmt->execute()) {
            $stmt->store_result();
            $stmt->bind_result($idOportunidad, $origen, $destino, $fecha, $estado, $modalidad);
