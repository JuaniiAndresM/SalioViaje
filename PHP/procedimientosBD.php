...
//UPDATE `salioviajeuy_salioviajeuy`.`faqs` SET `Respuesta` = 'respuesta' WHERE (`idPregunta` = '38');
    ...
    public function traer_oportunidades()
    {
        ...
        $oportunidades = array();
        $conn = $this->conexion();
        $query = "SELECT idViaje,Descuento,Origen,Destino,Fecha,Nombre,Apellido,Marca,Modelo,Capacidad,Estado,Matricula,Distancia,Precio,CantidadPasajeros,ESTADO_MTOP FROM viajes,usuarios,vehiculos where idTransportista = usuarios.ID and Vehiculo = Matricula and Modalidad = 'Oportunidad' and visivilidad != 0 ORDER BY Fecha;";
        $stmt = $conn->prepare($query);
        if ($stmt->execute()) {
            $stmt->store_result();
            $stmt->bind_result($idOportunidad, $descuento, $origen, $destino, $fecha, $nombre, $apellido, $marca, $modelo, $capacidad_vehiculo, $estado, $matricula, $distancia, $precio, $cantidad_pasajeros, $mtop);
            while ($stmt->fetch()) {
                $result = array('ID' => $idOportunidad, 'DESCUENTO' => $descuento, 'ORIGEN' => $origen, 'DESTINO' => $destino, 'FECHA' => $fecha, 'NOMBRE' => $nombre, 'APELLIDO' => $apellido, 'MARCA' => $marca, 'MODELO' => $modelo, 'CAPACIDAD_VEHICULO' => $capacidad_vehiculo, 'ESTADO' => $estado, 'MATRICULA' => $matricula, 'DISTANCIA' => $distancia, 'PRECIO' => $precio, "CANTIDAD_PASAJERO" => $cantidad_pasajeros, "MTOP" => $mtop);
                $fecha = $result["FECHA"];
    ...
    public function traer_oportunidades_por_id_usuario($id)
    {
        ...
        $oportunidades = array();
        $conn = $this->conexion();
        $query = "SELECT idViaje,Descuento,Origen,Destino,Fecha,Estado,Precio,CantidadPasajeros,Modalidad,idTransportista,Nombre,Telefono FROM viajes,usuarios where idComprador = $id and usuarios.ID = idTransportista and Modalidad = 'Oportunidad' and visivilidad != 0 ORDER BY Fecha";
        $stmt = $conn->prepare($query);
        if ($stmt->execute()) {
            $stmt->store_result();
            $stmt->bind_result($idOportunidad, $descuento, $origen, $destino, $fecha, $estado, $precio, $cantidad_pasajeros, $modalidad, $id_tta, $nombre, $telefono);
            while ($stmt->fetch()) {
                $result = array('ID' => $idOportunidad, 'DESCUENTO' => $descuento, 'ORIGEN' => $origen, 'DESTINO' => $destino, 'FECHA' => $fecha, 'ESTADO' => $estado, 'MODALIDAD' => $modalidad, "PRECIO" => $precio, "CANTIDAD_PASAJERO" => $cantidad_pasajeros, "NOMBRE" => $nombre, "TELEFONO" => $telefono);
                $fecha = $result["FECHA"];
                $timestamp = strtotime($fecha);
                $newDate = date("d-m-Y H:i", $timestamp);
                $result["FECHA"] = $newDate;
    ...
    public function traer_oportunidades_por_id_tta($id)
    {
        ...
        $oportunidades = array();
        $conn = $this->conexion();
        $query = "SELECT idViaje,Descuento,Origen,Destino,Fecha,Estado,Modalidad,id_viaje_vinculado,Distancia,Vehiculo,Nombre,Apellido FROM viajes,usuarios where idTransportista = $id and usuarios.ID = $id and visivilidad != 0 ORDER BY Fecha";
        $stmt = $conn->prepare($query);
        if ($stmt->execute()) {
            $stmt->store_result();
            $stmt->bind_result($idOportunidad, $descuento, $origen, $destino, $fecha, $estado, $modalidad, $id_viaje_vinculado, $distancia, $vehiculo, $nombre, $apellido/*,$nro_mtop, $pass_mtop*/);
            while ($stmt->fetch()) {
                $result = array('ID' => $idOportunidad, 'DESCUENTO' => $descuento, 'ORIGEN' => $origen, 'DESTINO' => $destino, 'FECHA' => $fecha, 'ESTADO' => $estado, 'MODALIDAD' => $modalidad, 'ID_VIAJE_VINCULADO' => $id_viaje_vinculado, "DISTANCIA" => $distancia, "VECHICULO" => $vehiculo, "NOMBRE" => $nombre, "APELLIDO" => $apellido, "NRO_MTOP" => $nro_mtop, "PASS_MTOP" => $pass_mtop);
                $fecha = $result["FECHA"];
                $timestamp = strtotime($fecha);
                $newDate = date("d-m-Y H:i", $timestamp);
                $result["FECHA"] = $newDate;
    ...
    public function traer_oportunidades_por_id_tta_seccion_facturacion_admin()
    {
    ...
    public function traer_viajes()
    {
        $viajes = array();
        $conn = $this->conexion();
        $query = "SELECT idViaje,Descuento,Origen,Destino,Fecha,Nombre,Apellido,Marca,Modelo,CantidadPasajeros,Estado,Matricula,Distancia,Precio,id_solicitud FROM viajes,usuarios,vehiculos where idTransportista = usuarios.ID and Vehiculo = Matricula and Modalidad = 'Agendado' and visivilidad != 0;";
        $stmt = $conn->prepare($query);
        if ($stmt->execute()) {
            $stmt->store_result();
            $stmt->bind_result($id, $descuento, $origen, $destino, $fecha, $nombre, $apellido, $marca, $modelo, $cantidad_pasajeros, $estado, $matricula, $distancia, $precio, $id_solicitud);
            while ($stmt->fetch()) {
                $result = array('ID' => $id, 'DESCUENTO' => $descuento, 'ORIGEN' => $origen, 'DESTINO' => $destino, 'FECHA' => $fecha, 'NOMBRE' => $nombre, 'APELLIDO' => $apellido, 'MARCA' => $marca, 'MODELO' => $modelo, 'CANTIDAD_PASAJEROS' => $cantidad_pasajeros, 'ESTADO' => $estado, 'MATRICULA' => $matricula, 'DISTANCIA' => $distancia, 'PRECIO' => $precio, 'ID_SOLICITUD' => $id_solicitud);
                $fecha = $result["FECHA"];
                $timestamp = strtotime($fecha);
                $newDate = date("d-m-Y H:i", $timestamp);
                $result["FECHA"] = $newDate;
    ...
    public function traer_agenda_usuario($id)
    {
        ...
        if ($stmt->execute()) {
            $stmt->store_result();
            $stmt->bind_result($id, $vehiculo, $distancia, $cantidad_pasajeros, $fecha, $origen, $destino, $precio, $estado, $modalidad, $id_transportista, $id_tramo_vinculado, $nombre, $apellido, $telefono, $id_solicitud, $estado_mtop);
            while ($stmt->fetch()) {
                $result = array('ID' => $id, 'VEHICULO' => $vehiculo, 'DISTANCIA' => $distancia, 'CANTIDAD_PASAJERO' => $cantidad_pasajeros, 'FECHA' => $fecha, 'ORIGEN' => $origen, 'DESTINO' => $destino, 'PRECIO' => $precio, 'RUTAS' => $rutas, 'ESTADO' => $estado, 'MODALIDAD' => $modalidad, 'ID_TRANSPORTISTA' => $id_transportista, "NOMBRE" => $nombre, "TELEFONO" => $telefono, "ID_SOLICITUD" => $id_solicitud, "MTOP" => $estado_mtop);
                $fecha = $result["FECHA"];
                $timestamp = strtotime($fecha);
...
//
    ...
    public function traer_agenda_usuario_no_tta($id)
    {
        ...
        $size = 0;
        $conn = $this->conexion();
        $query = "SELECT idViaje,Descuento,Vehiculo,Distancia,CantidadPasajeros,Fecha,Origen,Destino,Precio,Estado,Modalidad,idTransportista,id_viaje_vinculado,ESTADO_MTOP,id_solicitud, Nombre, Telefono FROM viajes, usuarios WHERE (usuarios.ID = idTransportista) AND ($id = idTransportista OR $id = idComprador) and visivilidad = 1 ORDER BY Fecha;";
        $stmt = $conn->prepare($query);
        if ($stmt->execute()) {
            $stmt->store_result();
            $stmt->bind_result($id, $descuento, $vehiculo, $distancia, $cantidad_pasajeros, $fecha, $origen, $destino, $precio, $estado, $modalidad, $id_transportista, $id_tramo_vinculado, $mtop, $id_solicitud,$nombre, $telefono);
            while ($stmt->fetch()) {
                $result = array('ID' => $id, 'DESCUENTO' => $descuento, 'VEHICULO' => $vehiculo, 'DISTANCIA' => $distancia, 'CANTIDAD_PASAJERO' => $cantidad_pasajeros, 'FECHA' => $fecha, 'ORIGEN' => $origen, 'DESTINO' => $destino, 'PRECIO' => $precio, 'RUTAS' => $rutas, 'ESTADO' => $estado, 'MODALIDAD' => $modalidad, 'ID_TRANSPORTISTA' => $id_transportista, "MTOP" => $mtop, "ID_SOLICITUD" => $id_solicitud, "NOMBRE" => $nombre, "TELEFONO" => $telefono);
                $fecha = $result["FECHA"];
                $timestamp = strtotime($fecha);
                $newDate = date("d-m-Y H:i A", $timestamp);
                $result["FECHA"] = $newDate;
    ...
    public function agregar_cotizacion($datos, $tipo, $id_solicitante)
    {
        ...
        switch ($tipo) {
            case "traslados":
                ...
                $stmt->bind_param("ssssssssssssiisssisisssissi", $datos["DIRECCION_ORIGEN"], $datos["BARRIO_ORIGEN"], $datos["LOCALIDAD_ORIGEN"], $datos["DIRECCION_DESTINO"], $datos["BARRIO_DESTINO"], $datos["LOCALIDAD_DESTINO"], $datos["NULL"], $datos["NULL"], $datos["NULL"], $datos["NULL"], $datos["NULL"], $datos["NULL"], $datos["MASCOTAS"], $datos["CANTIDAD_PASAJEROS"], $datos["HORA"], $datos["OBSERVACIONES"], $datos['FECHA_SALIDA'], $datos['NULL'], $datos['NULL'], $datos['NULL'], $datos["TIPO"], $datos['NULL'], $datos['NULL'], $datos['NULL'], $datos['NULL'], $datos['NULL'], $datos['ID_SOLICITANTE']);
                break;
            case 'tour':
                $datos["TIPO"] = "Tour";
                $stmt->bind_param("ssssssssssssiisssisisssissi", $datos["DIRECCION_SALIDA_TOUR"], $datos["BARRIO_TOUR"], $datos["LOCALIDAD_TOUR"], $datos["NULL"], $datos["CIUDAD"], $datos["LOCALIDAD_DESTINO"], $datos["NULL"], $datos["NULL"], $datos["NULL"], $datos["NULL"], $datos["NULL"], $datos["NULL"], $datos["MASCOTA"], $datos["CANTIDAD_PASAJEROS"], $datos["HORA"], $datos["OBSERVACIONES"], $datos['FECHA_SALIDA'], $datos['DURACION'], $datos['NULL'], $datos['NULL'], $datos["TIPO"], $datos['NULL'], $datos['NULL'], $datos['NULL'], $datos['NULL'], $datos['NULL'], $datos['ID_SOLICITANTE']);
                break;
            case 'transferIn':
                $stmt->bind_param("ssssssssssssiisssisisssissi", $datos["DIRECCION_ORIGEN"], $datos["BARRIO_ORIGEN"], $datos["PUNTO_ORIGEN"], $datos["DIRECCION_DESTINO"], $datos["BARRIO_DESTINO"], $datos["LOCALIDAD_DESTINO"], $datos["NULL"], $datos["NULL"], $datos["NULL"], $datos["NULL"], $datos["NULL"], $datos["NULL"], $datos["MASCOTAS"], $datos["CANTIDAD_PASAJEROS"], $datos["HORA"], $datos["OBSERVACIONES"], $datos['FECHA_ARRIBO'], $datos['NULL'], $datos['NRO_VUELO_BARCO'], $datos['EQUIPAJE'], $datos['TIPO_TRANSFER'], $datos['NULL'], $datos['NULL'], $datos['NULL'], $datos['NULL'], $datos['NULL'], $datos['ID_SOLICITANTE']);
                break;
            case 'transferOut':
                $stmt->bind_param("ssssssssssssiisssisisssissi", $datos["DIRECCION_ORIGEN"], $datos["BARRIO_ORIGEN"], $datos["LOCALIDAD_ORIGEN"], $datos["DIRECCION_DESTINO"], $datos["BARRIO_DESTINO"], $datos["PUNTO_DESTINO"], $datos["NULL"], $datos["NULL"], $datos["NULL"], $datos["NULL"], $datos["NULL"], $datos["NULL"], $datos["MASCOTAS"], $datos["CANTIDAD_PASAJEROS"], $datos["HORA"], $datos["OBSERVACIONES"], $datos['FECHA_PARTIDA'], $datos['NULL'], $datos['NRO_VUELO_BARCO'], $datos['EQUIPAJE'], $datos['TIPO_TRANSFER'], $datos['NULL'], $datos['NULL'], $datos['NULL'], $datos['NULL'], $datos['NULL'], $datos['ID_SOLICITANTE']);
                break;
            case 'fiestasIda':
                $stmt->bind_param("ssssssssssssiisssisisssissi", $datos["DIRECCION_ORIGEN"], $datos["BARRIO_ORIGEN"], $datos["LOCALIDAD_ORIGEN"], $datos["PUNTO_DESTINO"], $datos["BARRIO_DESTINO"], $datos["PUNTO_DESTINO"], $datos["NULL"], $datos["NULL"], $datos["NULL"], $datos["NULL"], $datos["NULL"], $datos["NULL"], $datos["MASCOTAS"], $datos["CANTIDAD_PASAJEROS_IDA"], $datos["HORA"], $datos["OBSERVACIONES"], $datos['FECHA_SALIDA'], $datos['NULL'], $datos['NULL'], $datos['NULL'], $datos['TRAMOS_FIESTA'], $datos['NULL'], $datos['NULL'], $datos['NULL'], $datos['NULL'], $datos['NULL'], $datos['ID_SOLICITANTE']);
                break;
            case 'fiestasVuelta':
                $stmt->bind_param("ssssssssssssiisssisisssissi", $datos["PUNTO_ORIGEN"], $datos["BARRIO_ORIGEN"], $datos["PUNTO_ORIGEN"], $datos["DIRECCION_DESTINO"], $datos["BARRIO_DESTINO"], $datos["LOCALIDAD_DESTINO"], $datos["NULL"], $datos["NULL"], $datos["NULL"], $datos["NULL"], $datos["NULL"], $datos["NULL"], $datos["MASCOTAS"], $datos["CANTIDAD_PASAJEROS_VUELTA"], $datos["HORA"], $datos["OBSERVACIONES"], $datos['FECHA_REGRESO'], $datos['NULL'], $datos['NULL'], $datos['NULL'], $datos['TRAMOS_FIESTA'], $datos['NULL'], $datos['NULL'], $datos['NULL'], $datos['NULL'], $datos['NULL'], $datos['ID_SOLICITANTE']);
                break;
            case 'fiestasIdaVuelta':
                $stmt->bind_param("ssssssssssssiisssisisssissi", $datos["DIRECCION_ORIGEN"], $datos["BARRIO_ORIGEN"], $datos["LOCALIDAD_ORIGEN"], $datos["DIRECCION_DESTINO"], $datos["BARRIO_DESTINO"], $datos["LOCALIDAD_DESTINO"], $datos["DIRECCION_ORIGEN_VUELTA"], $datos["BARRIO_ORIGEN_VUELTA"], $datos["LOCALIDAD_ORIGEN_VUELTA"], $datos["DIRECCION_DESTINO_VUELTA"], $datos["BARRIO_DESTINO_VUELTA"], $datos["LOCALIDAD_DESTINO_VUELTA"], $datos["MASCOTAS"], $datos["CANTIDAD_PASAJEROS_IDA"], $datos["HORA_SALIDA"], $datos["OBSERVACIONES"], $datos['FECHA_SALIDA'], $datos['NULL'], $datos['NULL'], $datos['NULL'], $datos['TRAMOS_FIESTA'], $datos['PUNTO_DESTINO'], $datos['PUNTO_SALIDA'], $datos['CANTIDAD_PASAJEROS_VUELTA'], $datos['HORA_REGRESO'], $datos['FECHA_REGRESO'], $datos['ID_SOLICITANTE']);
                break;
        }
        if ($stmt->execute()) {
            $stmt->store_result();
    ...
    public function traer_preferencias_por_id_tta($idtta)
    {
        $preferencias = array();
        $conn = $this->conexion();
        $query = "SELECT MATRICULA,NOCTURNO,FIESTAS,DIA_LIBRE,PRECIO_DE_COCHE,id_tta FROM `prefecrenciasVehiculos` WHERE id_tta = $idtta;";
        $stmt = $conn->prepare($query);
        if ($stmt->execute()) {
            $stmt->store_result();
            $stmt->bind_result($matricula,$nocturno, $fiestas, $dia_libre, $precio_coche, $idtta);
    ...
    public function obtener_preferencias_vehiculo($matricula){
        $preferencias = array();
        $conn = $this->conexion();
        $query = "SELECT * FROM `prefecrenciasVehiculos` where MATRICULA = '$matricula'; ";
        $stmt = $conn->prepare($query);
        if ($stmt->execute()) {
            $stmt->store_result();
            $stmt->bind_result($matricula, $nocturno, $fiestas, $dia_libre, $precio_coche, $id_tta);
            while ($stmt->fetch()) {
                $result = array("NOCTURNO" => $nocturno, "FIESTAS" => $fiestas, "DIA_LIBRE" => $dia_libre, "PRECIO_COCHE" => $precio_coche);
            }
        }
        $stmt->close();
        return json_encode($result);
    ...
    public function guardar_seleccion_de_transportistas_ya_notificados($idtta, $id_viaje, $matricula){
        $conn = $this->conexion();
        $query = "CALL guardar_top_30_notificados(?,?,?);";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("iis", $idtta, $id_viaje, $matricula);
        $stmt->execute();
        echo $stmt->error;
    ...
    //SELECT count(*) FROM `cotizaciones_presentadas` where ID_VIAJE_COTIZADO = 397

    public function numero_de_cotizaciones_presentadas_para_una_solicitud_de_viaje($id)
    {
        $conn = $this->conexion();
        $query = "SELECT count(*) FROM `cotizaciones_presentadas` where ID_VIAJE_COTIZADO = $id;";
        $stmt = $conn->prepare($query);
        if ($stmt->execute()) {
            $stmt->store_result();
    ...
    //select id from cotizaciones where ESTADO = 1 
    public function id_solicitudes_cotizando()
    {
        $id_viajes = array();
        $conn = $this->conexion();
        $query = "SELECT id FROM cotizaciones WHERE ESTADO = 1 ";
        ...
        $stmt->close();
        return json_encode($id_viajes);
    }

    //SELECT id_tta,Email FROM `top30`,`usuarios` where ID = ID_TTA;
    public function transportistas_top_30($id)
    {
        $top = array();
        $conn = $this->conexion();
        $query = "SELECT id_tta,Email FROM `top30`,`usuarios` where ID = ID_TTA and ID_VIAJE = $id and NOTIFICADO = 0;";
        $stmt = $conn->prepare($query);
        if ($stmt->execute()) {
            $stmt->store_result();
            $stmt->bind_result($id,$mail);
            while ($stmt->fetch()) {
                $result = array("ID" => $id, "MAIL" => $mail);
                $top[] = $result;
            }
        }
        $stmt->close();
    ...
    //UPDATE `top30` SET `NOTIFICADO`= 1 WHERE ID_TTA = $id_tta and ID_VIAJE = $id_viaje;

    public function set_notificado($idtta, $id_viaje){
        $conn = $this->conexion();
        $query = "UPDATE `top30` SET `NOTIFICADO`= 1 WHERE ID_TTA = $idtta and ID_VIAJE = $id_viaje;";
        $stmt = $conn->prepare($query);
        $stmt->execute();
        echo $stmt->error;
        $stmt->close();
    ...
    {
        $conn = $this->conexion();
        $query = "SELECT count(*) FROM `viajes` where Fecha >= '$OCUPADO_IDA' AND Fecha <= '$OCUPADO_VUELTA' AND idTransportista = $TTA;";
        $stmt = $conn->prepare($query);
        if ($stmt->execute()) {
            $stmt->store_result();
    ...
    public function no_mas_cotizaciones($VIAJE)
    {
        $conn = $this->conexion();
        $query = "SELECT COUNT(*) FROM `top30` WHERE ID_VIAJE = $VIAJE AND NOTIFICADO = 0;";
        $stmt = $conn->prepare($query);
        if ($stmt->execute()) {
            $stmt->store_result();
            $stmt->bind_result($NO_NOTIFICADOS);
            while ($stmt->fetch()) {
                $result = $NO_NOTIFICADOS;
            }
        }
        $stmt->close();
        return $result;
